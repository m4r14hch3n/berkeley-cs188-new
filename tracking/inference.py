...
def constructBayesNet(gameState: hunters.GameState):
    ...
    pacman_positions = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]
    ghost0_positions = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]
    ghost1_positions = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]

    obs0_values = set()
    obs1_values = set()
    for pacman_position in pacman_positions:
        for ghost0_position in ghost0_positions:
            distance_max = manhattanDistance(pacman_position, ghost0_position) + MAX_NOISE
            distance_min = max(0, manhattanDistance(pacman_position, ghost0_position) - MAX_NOISE)
            for i in range(distance_min, distance_max+1):
                obs0_values.add(i)

    for pacman_position in pacman_positions:
        for ghost1_position in ghost1_positions:
            distance_max = manhattanDistance(pacman_position, ghost1_position) + MAX_NOISE
            distance_min = max(0, manhattanDistance(pacman_position, ghost1_position) - MAX_NOISE)
            for i in range(distance_min, distance_max+1):
                obs1_values.add(i)


    variableDomainsDict = {}
    variableDomainsDict[PAC] = pacman_positions
    variableDomainsDict[GHOST0] = ghost0_positions
    variableDomainsDict[GHOST1] = ghost1_positions


    variableDomainsDict[OBS0] = list(obs0_values)
    variableDomainsDict[OBS1] = list(obs1_values)
